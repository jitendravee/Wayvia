import { directSearch, discoverJourneys, getHubCandidates } from "./discover";
...
async function fetchModeLegs(mode, from, to, opts) {
    if (mode === "train") {
...
function chainable(prev, next, buffer) {
 ...
 * All Modes toggle.
 */
export async function discoverMultimodal(from, to, opts) {
    const requestedModes = opts.modes ?? ALL_MODES;
    const trainRequested = requestedModes.includes("train");
    const otherModes = requestedModes.filter((m) => m !== "train" && Boolean(MODE_PROVIDERS[m]));
    ...
    const trainResult = trainRequested
    ...
    if (otherModes.length > 0) {
        ...
        await Promise.all(otherModes.map(async (mode) => {
    ...
    const legsByHubMode = new Map();
    if (usableHubs.length > 0) {
        await Promise.all(usableHubs.map(async (hub) => {
            const perMode = {};
            await Promise.all(requestedModes.map(async (mode) => {
                ...
                if (mode !== "train")
                    bump(mode, toHub.length + fromHub.length);
            }));
    ...
    if (maxConnections >= 2 && usableHubs.length >= 2) {
        ...
        const hubPairs = [];
        for (const a of threeLegHubs)
            for (const b of threeLegHubs)
                if (a !== b)
                    hubPairs.push([a, b]);
        // Middle hop hubA→hubB for each requested mode — fetched once per (pair, mode).
        const middleByPairMode = new Map();
        await Promise.all(hubPairs.flatMap(([a, b]) => requestedModes.map(async (mode) => {
            ...
            for (const modeA of requestedModes) {
                ...
                for (const modeB of requestedModes) {
                    ...
                    for (const modeC of requestedModes) {
                        ...
                        if (modeA === "train" && modeB === "train" && modeC === "train")
                            continue;
    ...
    const modesAvailable = [...(trainRequested ? ["train"] : []), ...otherModes];
    if (trainRequested)
        modeCounts.train = trainResult.direct.length + trainResult.viaHub.length + trainResult.viaTwoHub.length + trainResult.viaThreeHub.length;
