...
import { rankedCandidateHubs, DEFAULT_HUBS } from "./hubs";
import { discoverHubsFromRoutes } from "./dynamicHubs";
/**
...
function withAbsoluteTimes(leg, dayOffset) {
...
export async function directSearch(from, to, opts) {
 ...
 */
async function buildHubPool(liveCap = 150) {
    const pool = [...DEFAULT_HUBS];
...
async function fetchHubLegPairs(from, to, hubs, date) {
    return Promise.all(hubs.map(async (hub) => {
        ...
        const leg1s = leg1Result.success ? leg1Result.data : [];
        const leg2s = leg2Result.success ? leg2Result.data : [];
...
function connectLegPairs(legPairs, buffer, maxTransfer, hubSource) {
    const candidates = [];
    for (const { hub, leg1s, leg2s } of legPairs) {
        for (const e1 of leg1s) {
            ...
            for (const e2 of leg2s) {
                ...
                if (leg2.depAbsMin < leg1.arrAbsMin + buffer)
                    continue;
                if (leg2.depAbsMin - leg1.arrAbsMin > maxTransfer)
                    continue;
 ...
 */
export async function hubSearch(from, to, opts) {
    const date = isoToDDMMYYYY(opts.date);
 ...
 */
export async function getHubCandidates(from, to, maxHubs = 10) {
    const pool = await buildHubPool();
 ...
 */
async function dynamicHubSearch(from, to, seedTrainNos, opts) {
    const date = isoToDDMMYYYY(opts.date);
    const buffer = opts.transferBufferMin ?? 45;
    const maxTransfer = opts.maxTransferMin ?? 8 * 60;
    const dynamicHubs = await discoverHubsFromRoutes(from, to, seedTrainNos, { maxSeedTrains: 5, maxStopsPerTrain: 8 });
    if (dynamicHubs.length === 0)
        return { candidates: [], hubsExplored: [] };
 ...
 */
export async function twoHubSearch(from, to, hubs, opts, maxPairs = 6) {
    const date = isoToDDMMYYYY(opts.date);
    const buffer = opts.transferBufferMin ?? 45;
    const maxTransfer = opts.maxTransferMin ?? 8 * 60;
    ...
    const pairs = [];
    for (const a of top) {
        if (!(leg1ByHub.get(a.code) ?? []).length)
            continue;
        for (const b of top) {
            if (a.code === b.code)
                continue;
            if (!(leg3ByHub.get(b.code) ?? []).length)
                continue;
            pairs.push({ a, b });
        }
    }
    const middleLegs = await Promise.all(pairs.map(async ({ a, b }) => {
    ...
    for (const { a, b, entries } of middleLegs) {
        ...
        for (const e1 of leg1s) {
            ...
            for (const e2raw of entries) {
                ...
                if (leg2.depAbsMin < leg1.arrAbsMin + buffer)
                    leg2 = withAbsoluteTimes(entryToLeg(e2raw), 1);
                if (leg2.depAbsMin < leg1.arrAbsMin + buffer)
                    continue;
                if (leg2.depAbsMin - leg1.arrAbsMin > maxTransfer)
                    continue;
                for (const e3raw of leg3s) {
                    ...
                    if (leg3.depAbsMin < leg2.arrAbsMin + buffer)
                        continue;
                    if (leg3.depAbsMin - leg2.arrAbsMin > maxTransfer)
                        continue;
 ...
 */
export async function threeHubSearch(from, to, hubs, opts, maxPerHop = 4) {
    const date = isoToDDMMYYYY(opts.date);
    const buffer = opts.transferBufferMin ?? 45;
    const maxTransfer = opts.maxTransferMin ?? 8 * 60;
    ...
    if (abPairs.length === 0)
        return [];
    const abMiddle = await Promise.all(abPairs.flatMap((a) => top
    ...
    const bcMiddle = await Promise.all(bcPairs.flatMap((b) => cCandidates
                ...
                for (const { c, entries: bcEntries } of bcOptions) {
                    ...
                    for (const e3raw of bcEntries) {
                        ...
                        for (const e4raw of leg4s) {
                            ...
                            if (leg4.depAbsMin < leg3.arrAbsMin + buffer)
                                continue;
                            if (leg4.depAbsMin - leg3.arrAbsMin > maxTransfer)
                                continue;
                            candidates.push({
 ...
 */
function buildPartialCoverage(legPairs, max = 5) {
    const out = [];
    const seen = new Set();
 ...
 */
export async function discoverJourneys(from, to, opts) {
    ...
    const maxConnections = opts.maxConnections ?? (opts.forceTwoHub ? 2 : 2);
    ...
    if (runTierTwo) {
        ...
        const [dynamicResult, twoHub] = await Promise.all([
            seedTrainNos.length > 0
                ...
                : Promise.resolve({ candidates: [], hubsExplored: [] }),
    ...
    const edgesDiscovered = direct.reduce((n, c) => n + c.legs.length, 0) +
