 ...
 */
const TRAIN_NAME_HINTS = /\b(EXPRESS|EXP|MAIL|PASSENGER|PASS|SPECIAL|SPL|SUPERFAST|SF|RAJDHANI|SHATABDI|DURONTO|GARIB\s*RATH|HUMSAFAR|JANSHATABDI|JAN\s*SHATABDI|VANDE\s*BHARAT|INTERCITY|SUVIDHA|ANTYODAYA|TEJAS|SAMPARK\s*KRANTI|YUVA|UDAY|AC\s*EXP)\b/i;
export function looksLikeStation(rec) {
...
function tryJsonShape(text) {
    ...
    const arr = Array.isArray(json)
        ...
        : json && typeof json === "object"
            ? json.data ?? json.stations ?? json.list
            : null;
...
export function registerDiscoveredStations(stops) {
    for (const s of stops) {
        const code = s.code?.trim().toUpperCase();
        const name = s.name?.trim();
        if (!code || !name)
            continue;
        if (!looksLikeStation({ code, name }))
            continue;
...
export function getDiscoveredStations() {
    return Array.from(discovered.values());
}
export function discoveredStationCount() {
