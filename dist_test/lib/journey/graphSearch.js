...
import { allCachedPlaces, getOrCreatePlace } from "../places/repository";
...
async function resolvePlaceFromStationCode(stationCode) {
    try {
        ...
        const place = await getOrCreatePlace(cityName);
        // Add resolved place to the general places cache
        if (place) {
            const placeCache = PlaceCache.getInstance();
            placeCache.addGeneralPlace(place);
        }
        return place;
    }
    catch (err) {
 ...
 *
 * @param externalCandidates Optional array of Place objects to use as the base
 *                           pool for geography-based candidate generation.
 *                           If provided, these places are used in addition to
 *                           the origin and destination (which are removed from
 *                           the pool). If not provided, falls back to the
 *                           existing behavior (DEFAULT_HUBS ∪ allCachedPlaces).
 */
export async function multimodalGraphSearch(origin, destination, date, filters, externalCandidates) {
    ...
    function fetchEdges(from, to, mode) {
    ...
    async function expand(state) {
        ...
        if (!isLastAllowedHop) {
            ...
            for (const mode of filters.modes) {
                try {
                    ...
                    const geographicTargets = await rankedPlaceNeighbors(state.currentPlace, destination, NEIGHBOR_POOL_FIRST_HOP * 2, externalCandidates);
                    // Also always include the destination for direct connection discovery
                    const targetPlaces = new Set([destination, ...geographicTargets]);
                    ...
                    for (let i = 0; i < Math.min(4, cachedPlaces.length); i++) {
                        ...
                        if (place.id !== state.currentPlace.id && !targetPlaces.has(place)) {
                            ...
                            // Add cached place to the general places cache for future use
                            const placeCache = PlaceCache.getInstance();
                            placeCache.addGeneralPlace(place);
                        }
                    }
                    ...
                    const connectionsPromises = Array.from(targetPlaces).map(async (target) => {
                        try {
                            ...
                            return resolvedPlaces.filter((p) => p !== null);
                        }
                        catch (err) {
                    ...
                    for (const placesArray of connectionsResults) {
                        for (const place of placesArray) {
                            ...
                            if (place && place.id !== state.currentPlace.id && !state.visitedPlaceIds.has(place.id)) {
                                ...
                                // Add discovered place to the general places cache for future use
                                const placeCache = PlaceCache.getInstance();
                                placeCache.addGeneralPlace(place);
                            }
                        }
        ...
        const geographyCandidates = isLastAllowedHop
            ? [destination]
            : dedupeById([destination, ...(await rankedPlaceNeighbors(state.currentPlace, destination, NEIGHBOR_POOL_FIRST_HOP, externalCandidates))]).filter((p) => !state.visitedPlaceIds.has(p.id));
        ...
        for (const place of geographyCandidates) {
            if (place && combinedCandidates.length >= targetPoolSize)
                break;
            if (place && !seenIds.has(place.id)) {
                seenIds.add(place.id);
                combinedCandidates.push(place);
        ...
        const candidates = combinedCandidates.filter((p) => p !== null);
        await Promise.all(candidates.map(async (candidate) => {
            await Promise.all(filters.modes.map(async (mode) => {
                ...
                for (const leg of legs) {
                    if (prevLeg) {
                        if (!canConnect(prevLeg, leg, filters))
                            continue;
                    }
                    ...
                    if (!withinBudget(newCost, filters))
                        continue;
                    ...
                    if (!withinDuration(newDuration, filters))
                        continue;
                    if (candidate.id === destination.id && !withinArrivalWindow(leg, filters))
                        continue;
                    const nextState = {
