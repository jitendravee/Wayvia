 ...
 */
export function scoreAndFilterConnections(connections, filters, origin, destination) {
 ...
 */
function calculateConnectionScore(leg, origin, destination, filters) {
 ...
 */
function calculateGeographicScore(leg, origin, destination) {
    if (!origin.hasCoords || !destination.hasCoords)
        return 0.2; // Neutral when coords missing
 ...
 */
function calculateScheduleScore(leg, filters) {
    let score = 0.15; // Base score
    // Prefer reasonable departure times (not too early/late)
    const hour = new Date(leg.departure.split(':')[0]).getHours(); // Simplified
 ...
 */
function calculateProviderScore(leg) {
 ...
 */
function calculateConstraintCompliance(leg, filters) {
 ...
 */
function calculateAdaptiveThreshold(scored) {
    if (scored.length < 2)
        return 0.3;
 ...
 */
function determineBeamWidth(maxConnections) {
