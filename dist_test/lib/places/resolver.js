 ...
 * normalizer.
 */
export function normalizePlaceName(rawName) {
...
export function placeIdFromName(rawName) {
...
function withBus(place, bus) {
    ...
    if (locations.some((b) => b.name.toLowerCase() === bus.name.toLowerCase()))
        return place;
 ...
 */
export function placeFromHubSeed(hub) {
...
export async function resolvePlace(query) {
    ...
    if (!place) {
        ...
        if (hubSeed) {
            ...
            if (geo)
                place = { ...place, latitude: geo.lat, longitude: geo.lon, hasCoords: true };
        }
    }
