 ...
 */
const byId = new Map();
const byQuery = new Map(); // raw query (lowercased) -> place id, so repeat lookups of the same typed text skip re-resolution entirely
const byStationCode = new Map(); // station code -> place id, populated as a side effect of every resolution
function index(place) {
    byId.set(place.id, place);
    for (const s of place.railway?.stations ?? [])
        byStationCode.set(s.code.toUpperCase(), place.id);
}
/** Resolves (and caches) the canonical Place for free-text input — a station code, station name, or plain city name. */
export async function getOrCreatePlace(query) {
    const key = query.trim().toLowerCase();
    if (!key)
        return null;
    const cachedId = byQuery.get(key);
    if (cachedId)
        return byId.get(cachedId) ?? null;
    ...
    // Also add to the general places cache for candidate generation
    PlaceCache.getInstance().addGeneralPlace(merged);
    return merged;
}
...
function mergeInto(existing, incoming) {
    ...
    for (const s of incoming.railway?.stations ?? [])
        if (!stations.some((x) => x.code === s.code))
            stations.push(s);
    const busLocations = [...(existing.bus?.locations ?? [])];
    for (const b of incoming.bus?.locations ?? [])
        if (!busLocations.some((x) => x.name.toLowerCase() === b.name.toLowerCase()))
            busLocations.push(b);
    return {
        ...existing,
...
export function getCachedPlaceByStationCode(code) {
 ...
 */
export function getOrCreateHubPlace(hub) {
...
export function allCachedPlaces() {
