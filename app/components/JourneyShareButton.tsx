...
export function generateWhatsAppOverview(journey: AnnotatedJourney): string {
  const origin = journey.legs[0]?.from || "Origin";
  const dest = journey.legs[journey.legs.length - 1]?.to || "Destination";
  const hubs = [journey.hub, journey.hub2, journey.hub3]
    .filter(Boolean)
    .join(" ➔ ");
  const duration = durationLabel(journey.totalDurationMin);
  const fare = journey.totalFare ? `₹${journey.totalFare}` : "N/A";
  ...
  if (hubs) {
    text += `📍 *Connecting Via:* ${hubs}\n`;
  }
  text += `⏱️ *Total Duration:* ${duration}\n`;
  text += `💺 *Status:* ${status}\n`;
  text += `💰 *Est. Fare:* ${fare}\n`;
  text += `─────────────────────────\n`;
  ...
  journey.legs.forEach((leg, idx) => {
    ...
    text += `\n*Leg ${idx + 1}:* ${modeEmoji} ${leg.trainName || leg.trainNo}\n`;
    text += `   ${leg.from} (${leg.departure}) ➔ ${leg.to} (${leg.arrival})\n`;
  ...
  text += `\n─────────────────────────\n`;
  text += `💡 *Found on Wayvia* — smart routes when direct trains are waitlisted!\n`;
...
export default function JourneyShareButton({
  ...
  const handleCopy = async (e: React.MouseEvent) => {
    e.stopPropagation();
    const text = generateWhatsAppOverview(journey);
    try {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <button
        ...
        title="Share route overview on WhatsApp"
        className="flex items-center gap-1.5 rounded-lg border border-emerald-500/30 bg-emerald-50/70 px-2.5 py-1.5 font-display text-[12px] font-semibold text-emerald-800 transition-all hover:bg-emerald-100 hover:border-emerald-500/60 active:scale-95 sm:px-3 sm:text-[12.5px]"
      >
        {/* WhatsApp Icon */}
        <svg
          viewBox="0 0 24 24"
