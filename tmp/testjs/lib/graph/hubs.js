...
exports.DEFAULT_HUBS = void 0;
exports.scoreHub = scoreHub;
exports.rankedCandidateHubs = rankedCandidateHubs;
exports.DEFAULT_HUBS = [
...
var HUBS_BY_CODE = new Map(exports.DEFAULT_HUBS.map(function (h) { return [h.code, h]; }));
 ...
 */
function scoreHub(hub, origin, destination) {
    var originStn = HUBS_BY_CODE.get(origin.toUpperCase());
    var destStn = HUBS_BY_CODE.get(destination.toUpperCase());
    ...
    var hubHasCoords = !(hub.lat === 0 && hub.lon === 0);
    if (!originStn || !destStn || !hubHasCoords) {
        return __assign(__assign({}, hub), { relevance: 0.5, detourRatio: null });
    }
    var direct = haversineKm(originStn, destStn);
    var viaOrigin = haversineKm(originStn, hub);
    var viaDest = haversineKm(hub, destStn);
    // Hub is basically on top of the origin or destination already - not a useful transfer.
    if (viaOrigin < 15 || viaDest < 15) {
        return __assign(__assign({}, hub), { relevance: 0, detourRatio: null });
    }
    var detourRatio = direct > 0 ? (viaOrigin + viaDest) / direct : 1;
    // detourRatio 1.0 = perfectly on the line. Penalize increasingly past that.
    var relevance = Math.max(0, 1 - (detourRatio - 1) * 0.8);
    return __assign(__assign({}, hub), { relevance: relevance, detourRatio: detourRatio });
}
/**
 ...
 */
function rankedCandidateHubs(from, to, max, hubs) {
    if (max === void 0) { max = 10; }
    if (hubs === void 0) { hubs = exports.DEFAULT_HUBS; }
    ...
    var byCode = new Map();
    for (var _i = 0, hubs_1 = hubs; _i < hubs_1.length; _i++) {
        var h = hubs_1[_i];
        if (!byCode.has(h.code))
            byCode.set(h.code, h);
    }
    return Array.from(byCode.values())
        .filter(function (h) { return h.code !== f && h.code !== t; })
        .map(function (h) { return scoreHub(h, f, t); })
        .filter(function (h) { return h.relevance > 0; })
