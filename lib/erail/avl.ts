...
export interface AvlAvailability {
  ...
  /** Seats available (AVAILABLE n) or waitlist position (n in "GNWL n"), if parseable. */
  count: number | null;
  rawStatus: string;
  rawNums: string;
}

export interface AvlFare {
  key: string; // without the trailing "_f"
  estimatedFare: number | null;
  raw: string[];
}

export interface AvlParsedResponse {
  availability: Map<string, AvlAvailability>;
  fares: Map<string, AvlFare>;
}

/** Builds the key erail.in/s.erail.in uses to identify a specific train+leg+class+quota+date. */
export function buildAvlKey(
  trainNo: string,
  from: string,
  to: string,
  travelClass: string,
  quota: string,
  date: { day: number; month: number } // month is 1-indexed (e.g. August = 8)
): string {
...
export function toAvlDate(isoDate: string): { day: number; month: number } {
  const [, mm, dd] = isoDate.split("-").map(Number);
  return { day: dd, month: mm };
}

function classifyStatus(status: string): { category: AvlStatusCategory; count: number | null } {
  const s = status.trim().toUpperCase();
  if (s === "NOT AVAILABLE") return { category: "NOT_AVAILABLE", count: null };
  if (s.startsWith("AVAILABLE")) {
    const m = s.match(/AVAILABLE\s+(\d+)/);
    return { category: "AVAILABLE", count: m ? Number(m[1]) : null };
  }
  if (s.startsWith("REGRET")) return { category: "REGRET", count: null };
  if (s.includes("RAC")) {
...
export function parseAvlResponse(rawDataField: string): AvlParsedResponse {
  const availability = new Map<string, AvlAvailability>();
  const fares = new Map<string, AvlFare>();

  const segments = rawDataField.split("~").filter((s) => s.length > 0);

  for (const segment of segments) {
    if (segment.startsWith("301^AVL_Response") || segment === "301") continue; // header segment
 ...
 *     checks it.
 */
export function buildAvlRequest(keys: string[]): { url: string; init: RequestInit } {
...
export async function fetchAvailability(keys: string[]): Promise<AvlParsedResponse> {
  ...
  let json: { action: string; data: string };
  try {
