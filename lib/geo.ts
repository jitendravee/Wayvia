...
/** Calculates distance in km between two stations by code/name */
export function stationDistanceKm(fromCode: string, toCode: string): number | null {
  const c1 = getStationCoord(fromCode);
  const c2 = getStationCoord(toCode);
  if (!c1 || !c2 || !c1.lat || !c2.lat) return null;
  return haversineDistanceKm(c1.lat, c1.lon, c2.lat, c2.lon);
}
